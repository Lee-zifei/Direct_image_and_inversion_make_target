...
class WindowAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = (k * v).sum(dim=-2, keepdim=True)
        attn = q * attn
        attn = self.attn_drop(attn)
        x = attn.reshape(B,S,N,C).reshape(B,S*N,C)
        x = self.proj(x)
        x = self.proj_drop(x)
        x = x.reshape(B,S,N,C)
        return x
...
class Hydranetv2(nn.Module):
    def __init__(self, img_size=128,  in_chans=1,num_classes=1,
        ...
        super(Hydranetv2,self).__init__()
        self.in_chans=in_chans
        ...
        self.encoder2 = residual_block(int(self.num_features*2**1))
        ...
        self.encoder1 = residual_block(int(self.num_features))
