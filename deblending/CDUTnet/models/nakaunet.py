...
class residual_block(nn.Module):
	def __init__(self,  dim=64,bias=False,norm_layer=partial(nn.BatchNorm2d, eps=1e-3)):
		...
		med_planes=dim//4
		self.layer1=nn.Conv2d(dim,med_planes,1,stride=1,padding=0,bias=bias)
		self.bn1=norm_layer(med_planes)
		self.ac1=nn.ReLU()
		self.layer2=nn.Conv2d(med_planes,med_planes,3,stride=1,padding=1,bias=bias)
		self.bn2=norm_layer(med_planes)
		self.ac2=nn.ReLU()
		self.layer3=nn.Conv2d(med_planes,dim,1,stride=1,padding=0,bias=bias)
		self.bn3=norm_layer(dim)
		self.ac3=nn.ReLU()
...
class NakaUnet(nn.Module):
	def __init__(self,  image_channels=1, kernel_size=3,norm_layer=partial(nn.BatchNorm2d, eps=1e-3),bias=False):
		...
		self.up1=nn.ConvTranspose2d(dim2,dim2,kernel_size,stride=2,padding=1,output_padding=1,bias=bias)
		self.reduce1=nn.Conv2d(dim3, dim2, kernel_size, stride=1, padding=1, bias=bias)
		self.bnr1=norm_layer(dim2)
		self.actr1 = nn.ReLU()
		self.residualu1=residual_block()

		self.up2=nn.ConvTranspose2d(dim2,dim2,kernel_size,stride=2,padding=1,output_padding=1,bias=bias)
		self.reduce2=nn.Conv2d(dim3, dim2, kernel_size, stride=1, padding=1, bias=bias)
		self.bnr2=norm_layer(dim2)
		self.actr2 = nn.ReLU()
		self.residualu2=residual_block()

		self.up3=nn.ConvTranspose2d(dim2,dim2,kernel_size,stride=2,padding=1,output_padding=1,bias=bias)
		self.reduce3=nn.Conv2d(dim3, dim2, kernel_size, stride=1, padding=1, bias=bias)
		self.bnr3=norm_layer(dim2)
		self.actr3 = nn.ReLU()
		self.residualu3=residual_block()

		self.up4=nn.ConvTranspose2d(dim2,dim2,kernel_size,stride=2,padding=1,output_padding=1,bias=bias)
		self.reduce4=nn.Conv2d(dim3, dim2, kernel_size, stride=1, padding=1, bias=bias)
		self.bnr4=norm_layer(dim2)
		self.actr4 = nn.ReLU()

		self.en1=nn.Conv2d(dim2, dim2, 1, stride=1, padding=0, bias=bias)
		self.enbn1=norm_layer(dim2)
		self.enact1 = nn.ReLU()
		self.output=nn.Conv2d(dim2, image_channels , 1, stride=1, padding=0, bias=bias)
