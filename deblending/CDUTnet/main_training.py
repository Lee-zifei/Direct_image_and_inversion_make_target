...
from subfunctions import normalize_patches,denormalize_patches,seis,normalize_input_and_label
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, lr_scheduler):
    ...
    for idx, (images, labels ,names) in enumerate(data_loader):
        ...
        samples, targets, norm_stats = normalize_input_and_label(samples, targets,True)
        # print(samples.shape)
        if config.DENOISE:
            index = index.cuda()
...
if __name__ == '__main__':
    ...
    pretrain=False
    # os.environ["WANDB_API_KEY"] = '52053a292986cda748fcfbc16748a9a5f50e2218'
    # os.environ.setdefault(
        # "WANDB_MODE", "disabled" if not config.WAMDB else "online")
    # wandb.login()
