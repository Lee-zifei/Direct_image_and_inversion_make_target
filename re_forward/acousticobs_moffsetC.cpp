...
//
//    Filename：acousticobs_moffsetC.cpp
...
//    Last modified:  2025-06-26 09:44:27
//    Modifier: Zu Shaohuan
//    Function：Modelling of acoustic wave equation
//
//==================================================================================
...
int main(int argc, char *argv[]) {
    ...
    float freq0;
    int ns, r_n;
    float sx0, shotdx, shotdep, moffsetx, s_distance, fdelay;
    ...
    if (access("forward_output", F_OK) == -1) {
    ...
    for (is = 0; is < ns; is++) {
        for (ip = 0; ip < ss[is].r_n; ip++) {
            ss[is].r_ix[ip] = pmlc + (int)(rx0 / dx) + (int)(recdx / dx) * ip;  // 18+is*16;//29+is*55;//
        }
        /*i=0;
        for(ix=0;ix<nx;ix++)
        {
        ...
        if(i>ss[is].r_n)
        {
            printf("The trace number of %d th source is out of
        range!\n",is+1); return(0);
        }*/
    }

    /*=========================================================
      Parameters of the second  sources...
      ========================================================*/
    if ((fp = fopen("./input/fshot_dela.dat", "rb")) == NULL)
        printf("Cannot read the dithering file\n");
    else {
        for (is = 0; is < ns; is++) {
            fread(&fdelay, sizeof(float), 1, fp);
            ss[is].s_delay = int(fdelay / dt);
            ss[is].distance = i_distance;
        }
    }
    fclose(fp);
    ...
    */
    ...
    ////////============================////////
    variables_malloc(ntx, ntz, ntp, nx, nz, pml, Lc, dx, dz, itmax, plan, GPU_N, rnmax, NNmax);
    ...
    if (myid == 0) {
        ...
        fp = fopen("./forward_output/acc_vp.bin", "wb");
        ...
        fp = fopen("./forward_output/acc_rho.bin", "wb");
        for (ix = pmlc; ix <= ntx - pmlc - 1; ix++) {
            for (iz = pmlc; iz <= ntz - pmlc - 1; iz++) {
    ...
    printf("Obtain the direct wave seismogram !\n");

    inv_flag = 1;

    for (ip = 0; ip < ntp; ip++) {
        vp[ip] = vp[pmlc * ntx + pmlc];
        rho[ip] = rho[pmlc * ntx + pmlc];
    }

    for (iss = 0; iss < eachsid; iss++) {
        is = offsets + iss * GPU_N;
        fdtd_2d_GPU_forward(ntx, ntz, ntp, nx, nz, pml, Lc, rc, dx, dz, rick, itmax, dt, is, ss, plan, GPU_N, rnmax, rho, vp, k_x, k_x_half, k_z, k_z_half, a_x, a_x_half, a_z, a_z_half, b_x, b_x_half, b_z, b_z_half, inv_flag, 0, 10);

        for (i = 0; i < GPU_N; i++) {
            if (simu == 1)
                sprintf(filename, "./forward_output/%dsource_seismogram_dir_simu.bin", is + i + 1);
            else
                sprintf(filename, "./forward_output/%dsource_seismogram_dir.bin", is + i + 1);
    ...
    for (iss = 0; iss < eachsid; iss++) {
        ...
        if (simu == 1) {
            fdtd_2d_GPU_forward_simu(ntx, ntz, ntp, nx, nz, pml, Lc, rc, dx, dz, rick, itmax, dt, is, ss, plan, GPU_N, rnmax, rho, vp, k_x, k_x_half, k_z, k_z_half, a_x, a_x_half, a_z, a_z_half, b_x, b_x_half, b_z, b_z_half, inv_flag, save_wf, inter_wf);
        } else {
            fdtd_2d_GPU_forward(ntx, ntz, ntp, nx, nz, pml, Lc, rc, dx, dz, rick, itmax, dt, is, ss, plan, GPU_N, rnmax, rho, vp, k_x, k_x_half, k_z, k_z_half, a_x, a_x_half, a_z, a_z_half, b_x, b_x_half, b_z, b_z_half, inv_flag, save_wf, inter_wf);
        }
        for (i = 0; i < GPU_N; i++) {
            printf("The %5d shot forward is done!\n", is + i + 1);
            if (simu == 1)
                sprintf(filename, "./forward_output/%dsource_seismogram_dir_simu.bin", is + i + 1);
            else
                sprintf(filename, "./forward_output/%dsource_seismogram_dir.bin", is + i + 1);
            fp = fopen(filename, "rb");
            fread(&plan[i].seismogram_syn[0], sizeof(float), ss[is + i].r_n * itmax, fp);
            fclose(fp);

            for (ip = 0; ip < ss[is + i].r_n * itmax; ip++)
                plan[i].seismogram_rms[ip] = plan[i].seismogram_obs[ip] - plan[i].seismogram_syn[ip];
            if (simu == 1)
                sprintf(filename, "./forward_output/%dsource_seismogram_rms_simu.bin", is + i + 1);
            else
            ...
            if (simu == 1)
                sprintf(filename, "./forward_output/%dsource_seismogram_simu.bin", is + i + 1);
            else
                sprintf(filename, "./forward_output/%dsource_seismogram.bin", is + i + 1);
            fp = fopen(filename, "wb");
    ...
    variables_free(ntx, ntz, ntp, nx, nz, pml, Lc, dx, dz, itmax, plan, GPU_N, rnmax, NNmax);
...
/*==========================================================
  This suboutine is used for initializing the true model...
  ===========================================================*/
...
void ricker_wave(float *rick, int itmax, float f0, float t0, float dt, int flag) {
    ...
    if (flag == 3) {
        ...
        fp = fopen("./forward_output/rick_third_derive.bin", "wb");
        for (it = 0; it < itmax; it++) {
    ...
    if (flag == 2) {
        ...
        fp = fopen("./forward_output/rick_second_derive.bin", "wb");
        for (it = 0; it < itmax; it++) {
            fwrite(&rick[it], sizeof(float), 1, fp);
        }
        fclose(fp);
    ...
    if (flag == 1) {
        ...
        fp = fopen("./forward_output/rick_first_derive.bin", "wb");
        for (it = 0; it < itmax; it++) {
            fwrite(&rick[it], sizeof(float), 1, fp);
        }
        fclose(fp);
...
void get_ini_model(float *vp, float *rho, int ntp, int ntx, int ntz, int pml) {
    ...
    ///////////
    fp = fopen("../input/gradient_vp_all.bin", "rb");
    for (ix = pml; ix < ntx - pml; ix++) {
        for (iz = pml; iz < ntz - pml; iz++) {
            ip = iz * ntx + ix;
            fread(&vp[ip], sizeof(float), 1, fp);
