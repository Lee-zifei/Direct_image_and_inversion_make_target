...
__global__ void fdtd_cpml_2d_GPU_kernel_p(
		...
		int ntp, int ntx, int ntz,
		float *seismogram, int r_iz, int *r_ix, int r_n,
		...
		float *c, int inv_flag)
{
	...
	for(i=0;i<GPU_N;i++)
	{
		for(ip=0;ip<ntp;ip++)
		{
			...
			plan[i].phi_vx_x[ip]=0.0;
			plan[i].phi_vz_z[ip]=0.0;

			plan[i].phi_p_x[ip]=0.0;
			plan[i].phi_p_z[ip]=0.0;
		}
	}
	...
	for(i=0;i<GPU_N;i++)
	{
		...
		cudaMemcpyAsync(plan[i].d_vx,plan[i].vx,size_model,cudaMemcpyHostToDevice,plans[i].stream);
		cudaMemcpyAsync(plan[i].d_vz,plan[i].vz,size_model,cudaMemcpyHostToDevice,plans[i].stream);
		cudaMemcpyAsync(plan[i].d_p,plan[i].p,size_model,cudaMemcpyHostToDevice,plans[i].stream);

		cudaMemcpyAsync(plan[i].d_phi_vx_x,plan[i].phi_vx_x,size_model,cudaMemcpyHostToDevice,plans[i].stream);
		cudaMemcpyAsync(plan[i].d_phi_vz_z,plan[i].phi_vz_z,size_model,cudaMemcpyHostToDevice,plans[i].stream);

		cudaMemcpyAsync(plan[i].d_phi_p_x,plan[i].phi_p_x,size_model,cudaMemcpyHostToDevice,plans[i].stream);
		cudaMemcpyAsync(plan[i].d_phi_p_z,plan[i].phi_p_z,size_model,cudaMemcpyHostToDevice,plans[i].stream);
	}
	/////////////////////////////////
	// =============================================================================
		...
		for(i=0;i<GPU_N;i++)
		{
			...
			fdtd_cpml_2d_GPU_kernel_p<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
				 ...
				 ntp, ntx, ntz,
				 plan[i].d_seismogram, ss[is+i].r_iz, plan[i].d_r_ix, ss[is+i].r_n,
			...
			{
				cudaMemcpyAsync(plan[i].p,plan[i].d_p,sizeof(float)*ntp,cudaMemcpyDeviceToHost,plans[i].stream);
				cudaStreamSynchronize(plans[i].stream);

				sprintf(filename,"./rtm_slice/%dsource_p%d.bin",is+i+1,it);     
				fp=fopen(filename,"wb");
				for(ix=pmlc;ix<ntx-pmlc;ix++)
				{
					for(iz=pmlc;iz<ntz-pmlc;iz++)
					{					
						fwrite(&plan[i].p[iz*ntx+ix],sizeof(float),1,fp);
					}
				}
				fclose(fp);
			}
...
*/
			...
			sprintf(filename,"./rtm_output/wavefield%d_itmax%d.bin",myid,i);
			fp=fopen(filename,"wb");
...
(
 ...
 float *image_pp, float *image_sources, float *image_receivers, 
 ...
 )
{
	...
	if(iz>=pmlc&&iz<=ntz-pmlc-1&&ix>=pmlc&&ix<=ntx-pmlc-1)
	{
		...
		image_sources[ip]+=p_inv[ip]*p_inv[ip];//dp_dt[ip]*dp_dt[ip];//
...
(
 float *vp, float *rho, float *fxz, float *fxzr,
 float *p_inv, float *dp_dt, float *vx_inv, float *vz_inv, float *p, float *vx, float *vz,
 ...
 )
{
	...
	int ip=iz*ntx+ix;
	int ipp=iz*ntx*thetan+ix*thetan;
...
/*
	...
	for(i=0;i<GPU_N;i++)
	{
		...
		cudaMemcpyAsync(plan[i].d_image_pp,plan[i].image_pp,size_model,cudaMemcpyHostToDevice,plans[i].stream);
		cudaMemcpyAsync(plan[i].d_image_ppcig,plan[i].image_ppcig,size_model*thetan,cudaMemcpyHostToDevice,plans[i].stream);
		...
		cudaMemcpyAsync(plan[i].dp_dt,plan[i].p,size_model,cudaMemcpyHostToDevice,plans[i].stream);
	...
	for(it=itmax-2;it>=0;it--)
	{
		for(i=0;i<GPU_N;i++)
		{
			...
			fdtd_cpml_2d_GPU_kernel_vx_backward<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
				 plan[i].d_rho, plan[i].d_a_x_half, plan[i].d_a_z,
				 plan[i].d_b_x_half, plan[i].d_b_z,
				 plan[i].d_vx, plan[i].d_p,
				 plan[i].d_phi_p_x,
				 ntp, ntx, ntz, -dx, -dz, dt,
			...
			fdtd_cpml_2d_GPU_kernel_p_backward<<<dimGrid,dimBlock,0,plans[i].stream>>>
				( 
				 ...
				 plan[i].d_seismogram_rms, ss[is+i].r_iz, plan[i].d_r_ix, ss[is+i].r_n, -dx, -dz, dt, itmax, it
				);
			...
			sum_image_GPU_kernel_image<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
				 ...
				 plan[i].d_image_pp, plan[i].d_image_sources, plan[i].d_image_receivers,
				 ntx, ntz, thetan, pml, Lc, plan[i].d_rc, dx, dz
				);
/*
			sum_image_GPU_kernel_imageCIG<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
				 plan[i].d_vp, plan[i].d_rho, plan[i].d_fxz, plan[i].d_fxzr,
				 plan[i].d_p_inv, plan[i].dp_dt, plan[i].d_vx_inv, plan[i].d_vz_inv, plan[i].d_p, plan[i].d_vx, plan[i].d_vz,
...
*/
			if(it%10==0)
			{
				cudaMemcpyAsync(plan[i].p,plan[i].d_p_inv,sizeof(float)*ntp,cudaMemcpyDeviceToHost,plans[i].stream);
				cudaStreamSynchronize(plans[i].stream);

				sprintf(filename,"./rtm_output/%dsource_p%d_reconstruct_forward.bin",is+i+1,it);
				fp=fopen(filename,"wb");
				for(ix=pmlc;ix<ntx-pmlc;ix++)
				{
					for(iz=pmlc;iz<ntz-pmlc;iz++)
				...
				sprintf(filename,"./rtm_output/%dsource_p%d_backward.bin",is+i+1,it);     
				fp=fopen(filename,"wb");
				for(ix=pmlc;ix<ntx-pmlc;ix++)
				{
					for(iz=pmlc;iz<ntz-pmlc;iz++)
...
{ 
	...
	{            
		for(it=0;it<itmax;it++)
		{
			vx[ix*NX+it].x=seismogram_rms[ix*itmax+it];//[it*nx+ix];
		}
	} 
