...
__global__ void fdtd_cpml_2d_GPU_kernel_p_simu(
		float *rick, float *vp, float *rho,
		float *a_x, float *a_z,
		float *b_x, float *b_z,
		float *vx, float *vz, float *p,
		...
		)
{
	...
	if (it >=s_delay)
	{
		if(iz==s_iz&& ix==s_ix+distance && s_ix+distance<=ntx-Lc)
		//if(iz==s_iz&& ix==s_ix+distance)
			p[ip] = p[ip] + rick[it-s_delay];
	}
...
void fdtd_2d_GPU_forward(int ntx, int ntz, int ntp, int nx, int nz,
		...
		float *b_z, float *b_z_half, int inv_flag,int save_wf, int inter_wf)
{
	int i,it,ip;
	int ix,iz;
	int pmlc=pml+Lc;
...
extern "C"
void fdtd_2d_GPU_forward_simu(int ntx, int ntz, int ntp, int nx, int nz,
		int pml, int Lc, float *rc, float dx, float dz,
		float *rick, int itmax, float dt,
		int is, struct Source ss[], struct MultiGPU plan[], int GPU_N, int rnmax, float *rho,
		float *vp,
		...
		float *b_z, float *b_z_half, int inv_flag,int save_wf,int inter_wf)
{
	...
	for(it=0;it<itmax;it++)
	{
		for(i=0;i<GPU_N;i++)
		{
			...
			fdtd_cpml_2d_GPU_kernel_p_simu<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
				 plan[i].d_rick, plan[i].d_vp, plan[i].d_rho,
				 plan[i].d_a_x, plan[i].d_a_z, plan[i].d_b_x, plan[i].d_b_z,
				 plan[i].d_vx, plan[i].d_vz, plan[i].d_p,
				 ...
				 plan[i].d_p_borders_up, plan[i].d_p_borders_bottom,
				 plan[i].d_p_borders_left, plan[i].d_p_borders_right,
				 ss[is+i].s_delay,ss[is+i].distance
				);
